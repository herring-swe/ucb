 ...
 *
 * @brief Vector container for struct ucb_str*.
 *
...
/**
 * @struct ucb_vector_str
 * @brief vector containing struct ucb_str*.
 *
...
{
    struct ucb_str** data; ///< pointer to vector data
 ...
 */
typedef bool (*ucb_vector_str_iter_func)(struct ucb_str* val, size_t index, void* user_data);

/// @name Lifecycle
/// @{

/**
 * @fn ucb_vector_str_new
 * @brief Creates a new empty vector containing struct ucb_str*.
 *
...
/**
 * @fn ucb_vector_str_insert_sorted
...
UCB_API ucb_vector_str* ucb_vector_str_new(void);

UCB_API void ucb_vector_str_free(ucb_vector_str* vec);

UCB_API void ucb_vector_str_free_full(ucb_vector_str* vec);

UCB_API bool ucb_vector_str_copy(ucb_vector_str* dst, const ucb_vector_str* src);

UCB_API ucb_vector_str* ucb_vector_str_clone(const ucb_vector_str* src);

UCB_API void ucb_vector_str_move(ucb_vector_str* dst, ucb_vector_str* src);

/* ---------------------------- Capacity and size --------------------------- */

UCB_API bool ucb_vector_str_reserve(ucb_vector_str* vec, size_t new_capacity);

static inline size_t ucb_vector_str_capacity(const ucb_vector_str* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->capacity;
}

static inline size_t ucb_vector_str_size(const ucb_vector_str* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->size;
}

static inline bool ucb_vector_str_is_empty(const ucb_vector_str* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->size == 0;
}

static inline void ucb_vector_str_clear(ucb_vector_str* vec)
{
    UCB_VERIFY_ARGS(vec);
    vec->size = 0;
}
...
UCB_API void ucb_vector_str_fit(ucb_vector_str* vec);

/* --------------------- Element access and modification -------------------- */

UCB_API void ucb_vector_str_insert(ucb_vector_str* vec, size_t index, struct ucb_str* data);

UCB_API struct ucb_str* ucb_vector_str_remove(ucb_vector_str* vec, size_t index);

UCB_API void ucb_vector_str_push_back(ucb_vector_str* vec, struct ucb_str* data);

UCB_API void ucb_vector_str_push_front(ucb_vector_str* vec, struct ucb_str* data);

UCB_API struct ucb_str* ucb_vector_str_pop_back(ucb_vector_str* vec);

UCB_API struct ucb_str* ucb_vector_str_pop_front(ucb_vector_str* vec);

static inline const struct ucb_str* ucb_vector_str_peek_back(const ucb_vector_str* vec)
{
    UCB_VERIFY_ARGS(vec && vec->size);
    return vec->data[vec->size - 1];
}

static inline struct ucb_str* ucb_vector_str_peek_front(const ucb_vector_str* vec)
{
    UCB_VERIFY_ARGS(vec && vec->size);
    return vec->data[0];
}

static inline struct ucb_str* ucb_vector_str_get(const ucb_vector_str* vec, size_t index)
{
    UCB_VERIFY_ARGS(vec && index < vec->size);
    return vec->data[index];
}

static inline void ucb_vector_str_set(ucb_vector_str* vec, size_t index, struct ucb_str* data)
{
    UCB_VERIFY_ARGS(vec && index < vec->size);
    vec->data[index] = data;
}

static inline void ucb_vector_str_swap(ucb_vector_str* vec, size_t idx1, size_t idx2)
{
    UCB_VERIFY_ARGS(vec && idx1 < vec->size && idx2 < vec->size && idx1 != idx2);
    struct ucb_str* tmp = vec->data[idx1];
    vec->data[idx1] = vec->data[idx2];
    vec->data[idx2] = tmp;
}

UCB_API size_t ucb_vector_str_foreach(ucb_vector_str* vec,
                                      ucb_vector_str_iter_func func,
                                      void* user_data);
