 ...
 *
 * @brief Vector container for double.
 *
...
/**
 * @struct ucb_vector_dbl
 * @brief vector containing double.
 *
 * This is a generated vector for double.
 */
typedef struct ucb_vector_dbl
{
 ...
 */
typedef bool (*ucb_vector_dbl_iter_func)(double* pval, size_t index, void* user_data);

/// @name Lifecycle
/// @{

/**
 * @fn ucb_vector_dbl_new
 * @brief Creates a new empty vector containing double.
 *
...
/**
 * @fn ucb_vector_dbl_free
 * @brief Frees the vector.
 * @param vec The vector to free.
 */

/**
 * @fn ucb_vector_dbl_copy
...
/**
 * @fn ucb_vector_dbl_insert_sorted
...
/**
 * @fn ucb_vector_dbl_insert_sorted_with
...
UCB_API ucb_vector_dbl* ucb_vector_dbl_new(void);

UCB_API void ucb_vector_dbl_free(ucb_vector_dbl* vec);

UCB_API bool ucb_vector_dbl_copy(ucb_vector_dbl* dst, const ucb_vector_dbl* src);

UCB_API ucb_vector_dbl* ucb_vector_dbl_clone(const ucb_vector_dbl* src);

UCB_API void ucb_vector_dbl_move(ucb_vector_dbl* dst, ucb_vector_dbl* src);

/* ---------------------------- Capacity and size --------------------------- */

UCB_API bool ucb_vector_dbl_reserve(ucb_vector_dbl* vec, size_t new_capacity);

static inline size_t ucb_vector_dbl_capacity(const ucb_vector_dbl* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->capacity;
}

static inline size_t ucb_vector_dbl_size(const ucb_vector_dbl* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->size;
}

static inline bool ucb_vector_dbl_is_empty(const ucb_vector_dbl* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->size == 0;
}

static inline void ucb_vector_dbl_clear(ucb_vector_dbl* vec)
{
    UCB_VERIFY_ARGS(vec);
    vec->size = 0;
}

UCB_API void ucb_vector_dbl_fit(ucb_vector_dbl* vec);

/* --------------------- Element access and modification -------------------- */

UCB_API void ucb_vector_dbl_insert(ucb_vector_dbl* vec, size_t index, double data);

UCB_API double ucb_vector_dbl_remove(ucb_vector_dbl* vec, size_t index);

UCB_API void ucb_vector_dbl_push_back(ucb_vector_dbl* vec, double data);

UCB_API void ucb_vector_dbl_push_front(ucb_vector_dbl* vec, double data);

UCB_API double ucb_vector_dbl_pop_back(ucb_vector_dbl* vec);

UCB_API double ucb_vector_dbl_pop_front(ucb_vector_dbl* vec);

static inline double ucb_vector_dbl_peek_back(const ucb_vector_dbl* vec)
{
    UCB_VERIFY_ARGS(vec && vec->size);
    return vec->data[vec->size - 1];
}

static inline double ucb_vector_dbl_peek_front(const ucb_vector_dbl* vec)
{
    UCB_VERIFY_ARGS(vec && vec->size);
    return vec->data[0];
}

static inline double ucb_vector_dbl_get(const ucb_vector_dbl* vec, size_t index)
{
    UCB_VERIFY_ARGS(vec && index < vec->size);
    return vec->data[index];
}

static inline void ucb_vector_dbl_set(ucb_vector_dbl* vec, size_t index, double data)
{
    UCB_VERIFY_ARGS(vec && index < vec->size);
    vec->data[index] = data;
}

static inline void ucb_vector_dbl_swap(ucb_vector_dbl* vec, size_t idx1, size_t idx2)
{
    UCB_VERIFY_ARGS(vec && idx1 < vec->size && idx2 < vec->size && idx1 != idx2);
    double tmp = vec->data[idx1];
    vec->data[idx1] = vec->data[idx2];
    vec->data[idx2] = tmp;
}

UCB_API size_t ucb_vector_dbl_foreach(ucb_vector_dbl* vec,
                                      ucb_vector_dbl_iter_func func,
                                      void* user_data);

/* ------------------------------ Find and sort ----------------------------- */

UCB_API void ucb_vector_dbl_sort(ucb_vector_dbl* vec);

UCB_API void ucb_vector_dbl_sort_with(ucb_vector_dbl* vec, ucb_cmp_func func);

UCB_API size_t ucb_vector_dbl_insert_sorted(ucb_vector_dbl* vec, double val);

UCB_API size_t ucb_vector_dbl_insert_sorted_with(ucb_vector_dbl* vec,
                                                 double val,
                                                 ucb_cmp_func func);

UCB_API ucb_ssize ucb_vector_dbl_find(const ucb_vector_dbl* vec, const double* val);
