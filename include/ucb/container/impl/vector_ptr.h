...
/**
 * @struct ucb_vector_ptr
 * @brief vector containing void*.
 *
 * This is a generated vector for void*.
 ...
 */
typedef bool (*ucb_vector_ptr_iter_func)(void* val, size_t index, void* user_data);

/// @name Lifecycle
/// @{

/**
 * @fn ucb_vector_ptr_new
 * @brief Creates a new empty vector containing void*.
 *
...
/**
 * @fn ucb_vector_ptr_insert_sorted
...
/**
 * @fn ucb_vector_ptr_insert_sorted_with
...
UCB_API ucb_vector_ptr* ucb_vector_ptr_new(void);

UCB_API void ucb_vector_ptr_free(ucb_vector_ptr* vec);

UCB_API bool ucb_vector_ptr_copy(ucb_vector_ptr* dst, const ucb_vector_ptr* src);

UCB_API ucb_vector_ptr* ucb_vector_ptr_clone(const ucb_vector_ptr* src);

UCB_API void ucb_vector_ptr_move(ucb_vector_ptr* dst, ucb_vector_ptr* src);

/* ---------------------------- Capacity and size --------------------------- */

UCB_API bool ucb_vector_ptr_reserve(ucb_vector_ptr* vec, size_t new_capacity);

static inline size_t ucb_vector_ptr_capacity(const ucb_vector_ptr* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->capacity;
}

static inline size_t ucb_vector_ptr_size(const ucb_vector_ptr* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->size;
}

static inline bool ucb_vector_ptr_is_empty(const ucb_vector_ptr* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->size == 0;
}

static inline void ucb_vector_ptr_clear(ucb_vector_ptr* vec)
{
    UCB_VERIFY_ARGS(vec);
    vec->size = 0;
}

UCB_API void ucb_vector_ptr_fit(ucb_vector_ptr* vec);

/* --------------------- Element access and modification -------------------- */

UCB_API void ucb_vector_ptr_insert(ucb_vector_ptr* vec, size_t index, void* data);

UCB_API void* ucb_vector_ptr_remove(ucb_vector_ptr* vec, size_t index);

UCB_API void ucb_vector_ptr_push_back(ucb_vector_ptr* vec, void* data);

UCB_API void ucb_vector_ptr_push_front(ucb_vector_ptr* vec, void* data);

UCB_API void* ucb_vector_ptr_pop_back(ucb_vector_ptr* vec);

UCB_API void* ucb_vector_ptr_pop_front(ucb_vector_ptr* vec);

static inline const void* ucb_vector_ptr_peek_back(const ucb_vector_ptr* vec)
{
    UCB_VERIFY_ARGS(vec && vec->size);
    return vec->data[vec->size - 1];
}

static inline void* ucb_vector_ptr_peek_front(const ucb_vector_ptr* vec)
{
    UCB_VERIFY_ARGS(vec && vec->size);
    return vec->data[0];
}

static inline void* ucb_vector_ptr_get(const ucb_vector_ptr* vec, size_t index)
{
    UCB_VERIFY_ARGS(vec && index < vec->size);
    return vec->data[index];
}

static inline void ucb_vector_ptr_set(ucb_vector_ptr* vec, size_t index, void* data)
{
    UCB_VERIFY_ARGS(vec && index < vec->size);
    vec->data[index] = data;
}

static inline void ucb_vector_ptr_swap(ucb_vector_ptr* vec, size_t idx1, size_t idx2)
{
    UCB_VERIFY_ARGS(vec && idx1 < vec->size && idx2 < vec->size && idx1 != idx2);
    void* tmp = vec->data[idx1];
    vec->data[idx1] = vec->data[idx2];
    vec->data[idx2] = tmp;
}

UCB_API size_t ucb_vector_ptr_foreach(ucb_vector_ptr* vec,
                                      ucb_vector_ptr_iter_func func,
                                      void* user_data);
