 ...
 *
 * @brief Vector container for int.
 *
...
/**
 * @struct ucb_vector_int
 * @brief vector containing int.
 *
 * This is a generated vector for int.
 */
typedef struct ucb_vector_int
{
    int* data;       ///< pointer to vector data
 ...
 */
typedef bool (*ucb_vector_int_iter_func)(int* pval, size_t index, void* user_data);

/// @name Lifecycle
/// @{

/**
 * @fn ucb_vector_int_new
 * @brief Creates a new empty vector containing int.
 *
...
/**
 * @fn ucb_vector_int_free
 * @brief Frees the vector.
 * @param vec The vector to free.
 */

/**
 * @fn ucb_vector_int_copy
...
/**
 * @fn ucb_vector_int_insert_sorted
...
/**
 * @fn ucb_vector_int_insert_sorted_with
...
UCB_API ucb_vector_int* ucb_vector_int_new(void);

UCB_API void ucb_vector_int_free(ucb_vector_int* vec);

UCB_API bool ucb_vector_int_copy(ucb_vector_int* dst, const ucb_vector_int* src);

UCB_API ucb_vector_int* ucb_vector_int_clone(const ucb_vector_int* src);

UCB_API void ucb_vector_int_move(ucb_vector_int* dst, ucb_vector_int* src);

/* ---------------------------- Capacity and size --------------------------- */

UCB_API bool ucb_vector_int_reserve(ucb_vector_int* vec, size_t new_capacity);

static inline size_t ucb_vector_int_capacity(const ucb_vector_int* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->capacity;
}

static inline size_t ucb_vector_int_size(const ucb_vector_int* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->size;
}

static inline bool ucb_vector_int_is_empty(const ucb_vector_int* vec)
{
    UCB_VERIFY_ARGS(vec);
    return vec->size == 0;
}

static inline void ucb_vector_int_clear(ucb_vector_int* vec)
{
    UCB_VERIFY_ARGS(vec);
    vec->size = 0;
}

UCB_API void ucb_vector_int_fit(ucb_vector_int* vec);

/* --------------------- Element access and modification -------------------- */

UCB_API void ucb_vector_int_insert(ucb_vector_int* vec, size_t index, int data);

UCB_API int ucb_vector_int_remove(ucb_vector_int* vec, size_t index);

UCB_API void ucb_vector_int_push_back(ucb_vector_int* vec, int data);

UCB_API void ucb_vector_int_push_front(ucb_vector_int* vec, int data);

UCB_API int ucb_vector_int_pop_back(ucb_vector_int* vec);

UCB_API int ucb_vector_int_pop_front(ucb_vector_int* vec);

static inline int ucb_vector_int_peek_back(const ucb_vector_int* vec)
{
    UCB_VERIFY_ARGS(vec && vec->size);
    return vec->data[vec->size - 1];
}

static inline int ucb_vector_int_peek_front(const ucb_vector_int* vec)
{
    UCB_VERIFY_ARGS(vec && vec->size);
    return vec->data[0];
}

static inline int ucb_vector_int_get(const ucb_vector_int* vec, size_t index)
{
    UCB_VERIFY_ARGS(vec && index < vec->size);
    return vec->data[index];
}

static inline void ucb_vector_int_set(ucb_vector_int* vec, size_t index, int data)
{
    UCB_VERIFY_ARGS(vec && index < vec->size);
    vec->data[index] = data;
}

static inline void ucb_vector_int_swap(ucb_vector_int* vec, size_t idx1, size_t idx2)
{
    UCB_VERIFY_ARGS(vec && idx1 < vec->size && idx2 < vec->size && idx1 != idx2);
    int tmp = vec->data[idx1];
    vec->data[idx1] = vec->data[idx2];
    vec->data[idx2] = tmp;
}

UCB_API size_t ucb_vector_int_foreach(ucb_vector_int* vec,
                                      ucb_vector_int_iter_func func,
                                      void* user_data);
