...
# Test that source glue strings are correctly expanded
def test_source_glue_expansion():
    p = Preprocessor()

    # Include a piece of source with some macros to be expanded.
    # Note the multiline macro expansion.
    
    p.define("RET_TYPE", "int")
    p.define("RET_VAL", "1")
    p.include("main.c", """

    #define SNAKE_JOIN(a, b) a##_##b
    #define FUNC_NAME(name) SNAKE_JOIN(name, RET_TYPE)

    RET_TYPE void FUNC_NAME(test)(void)
    {
        return RET_VAL;
    }
